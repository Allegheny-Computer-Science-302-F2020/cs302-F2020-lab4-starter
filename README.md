...
* [Laboratory Assignment Tasks](#laboratory-assignment-tasks)
  + [Using HTML and CSS to Implement and Style a Web Page](#using-html-and-css-to-implement-and-style-a-web-page)
...
- To create hypertext references that do and do not open new web browser tabs
...
`Allegheny-Computer-Science-302-Fall-2020/computer-science-302-fall-2020-lab-4-gkapfham`.
...
```
git clone git@github.com:Allegheny-Computer-Science-302-F2020/computer-science-302-fall-2020-lab-4-gkapfham.git
```
...
### Using HTML and CSS to Implement and Style a Web Page
...
repository. Importantly, please note that this assignment asks you to complete
a web site that is more sophisticated than the one that you implemented for the
previous laboratory assignments. Overall, you will need to read and resolve all
of the `TODO` markers in the provided HTML and CSS files. To start, you should
include the cascading style sheet (CSS) that controls, for instance, the font
sizes on the site. You can achieve this step by adding the code `<link
href="css/site.css" rel="stylesheet">` to the `<head>` region of the
`src/www/index.html` file. Note that you also need to have another line of
source code to load the `emoji.css` file and its features for including an
emoji in a web site. How would you write that line of HTML source code?

Next, you need to ensure that your web site can load the Montserrat and
Merriweather fonts from the [Google Web Fonts
archive](https://fonts.google.com/). To complete this task, you should follow
the instructions on the Google Web Fonts site and Chapter 4's "Dive Deeper"
content on this topic. Please note that you must modify both an HTML file and a
CSS file if you want the Merriweather font to apply to the headers and the
Montserrat font to appear for all of the body text. The following source code
segment gives an example of the source code that you would need in the CSS file
if you want to specify the Merriweather font for the headers of the HTML. Please
note that, to ensure that the font displays correctly you also need to, for
instance, import the font in the `<head>` region of the page and then also
import the `site.css` file in the `<head>` region.
...
Along with picking the font family, you should also ensure that the font size
for the `<h1>` tag is set to 46px. Now, you also need to style the
`<blockquote>` and `<footer>` tags that will appear in your web site. To learn
more about these CSS rules, make sure that you review Figures 4.18 and 4.20 in
the textbook and the text that describes them. Specifically, you should ensure
that every `<blockquote>` has a 1pt border on the top and left and that the
`<footer>` has a 1pt border on the bottom and the right. The following source
code segment furnishes an example of some CSS source code that will introduce
some of this type of formatting.
...
you are ready to add the five reviews of the photograph. The following source
code segment gives an example of the HTML source code that will create the first
review. You should include a customized version of this source code in your own
web site, changing the name, time, and emoji to appropriate values.
...
Ultimately, your web site should contain at least five photograph reviews that
follow the format given in the previous code segment. Finally, students should
check the correctness of their HTML by running the command `htmlhint
src/www/index.html` in their terminal window. You should also ensure that all
aspects of the page preview correctly when you load them in your web browser.

As you can see from the screenshot provided in your GitHub repository, your page
should also have a section with links to external sites that provide travel
tips. For instance, your site should include links to the web pages in the
following list.
...
Using the technique that you learned in a previous assignment, these links
should always open in a new browser tab. Finally, your CSS file needs to specify
that the ordered list has a 1pt border on the top and left. Make sure that the
CSS rules for the ordered list also specify the same margins and padding as was
done for the `<blockquote>`. You also need to ensure that your site correctly
uses the `<footer>` tag as illustrated by the following source code segment.
Remember, you will need to add CSS to the `site.css` file to ensure that the
page's footer has a border on the bottom and right, as seen in the screenshot of
the final version of the web page.
...
127.0.0.1 - - [29/Sep/2020 13:04:39] "GET / HTTP/1.1" 200 -
127.0.0.1 - - [29/Sep/2020 13:04:39] "GET /css/site.css HTTP/1.1" 200 -
127.0.0.1 - - [29/Sep/2020 13:04:39] "GET /css/emoji.css HTTP/1.1" 200 -
127.0.0.1 - - [29/Sep/2020 13:04:39] "GET /img/plane.jpg HTTP/1.1" 200 -
```
...
```
        ┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓
        ┃ Passed 29/29 (100%) of checks for cs302-F2020-lab4! ┃
        ┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛
```
...
```
git remote add download git@github.com:Allegheny-Computer-Science-302-F2020/cs302-F2020-lab4-starter.git
```
